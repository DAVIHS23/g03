...
function updateTreemap(data, selCountry) {
    ...
    svgTree.selectAll("rect")
        ...
        .style("stroke", "black")
        .style("fill-opacity", d => selCountry === d.data.country ? 1 : 0.8)
        .style("fill", d => color(d.data.continent))
    ...
    .on("click", function handle(d, i) {
        ...
        // }
        updateTreemap(data, selectedCountry);
        updateBarchart(selectedCountry, selectedContinent, selectedCountryName);
    });

    // add the text/label to the rectangels
    svgTree.selectAll("text .country")
        .data(root.leaves())
        .join("text")
